...
def file_dispose(file_path, engine):
    ...
    if "zip" in file_path:
        ...
        data_file = pd.read_excel(file_path, sheet_name="东莞成本")
    # 将数据插入到数据库中的"人工导入202402运费汇总"表中，如果表已存在则替换原有数据，不插入索引，每次插入500行数据 append|replace
    data_file.to_sql('人工导入240808东莞成本', con=engine, if_exists='replace', index=False, chunksize=500)
    # 输出"数据插入完成"
    print("数据插入完成")
...
# window
download_file_path = r"C:\Users\73769\Desktop\7月跑东莞成本(1).xlsx"
# mac
