...
,ylmf as (select 花费,日期,总成交金额 from  sales.ods_猫超_阿里妈妈_引力魔方 )


select zdx.日期,智多星消耗,智多星曝光量,智多星点击量,智多星点击成本,智多星加购数,智多星收藏数,智多星总成交笔数,智多星总成交金额
,淘客成交金额,淘宝客佣金,总销售额,支付商品件数,退款金额,直通车展现量,直通车点击量,直通车花费,直通车总成交金额,直通车总成交笔数,自播销售额,主播销售额 ,ylmf.花费 引力魔方花费,ylmf.总成交金额 引力魔方产出
from zdx
left join tke on zdx.日期 = tke.日期
left join tku on zdx.日期 = tku.日期
left join ztc on zdx.日期 = ztc.日期
left join ll  on  zdx.日期 = ll.日期
left join ylmf on zdx.日期 = ylmf.日期
order by zdx.日期 asc
