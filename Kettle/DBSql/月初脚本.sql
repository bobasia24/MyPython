...
select sum(total_cost), count(1),sum(issued_quantity)  from rpa.ods_01财务_rpa_金蝶云星空_销售出库单  where `date` like '2024-06%'
and shipment_organization = '深圳榴芒';

-- -----------------------------------------------------------------------

select sum(amount_excluding_tax),sum(pricing_quantity)
from rpa.ods_01财务_金蝶_应付单列表_月维度 where business_date like '2024-06%' and payment_organization = '深圳榴芒'
